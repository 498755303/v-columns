<script>
import { flattenData, getNodes } from './util';
  ...
  render(h) {
    ...
    return (
      <div class="columns-panel">
              ...
              {data.map((item, index) => {
                return (
                  <div class="columns-panel__content_box">
                    <p class="columns-panel__content_box_header">{item.text}</p>
                    <CascaderItem
                      key={index}
        ...
        {this.showSelect && (this.checkedData && this.checkedData.length) || (this.mustColumns && this.mustColumns.length) ? (
          <SelectPanel
  ...
  methods: {
    getCheckedData() {
      ...
      function step(nodes) {
        ...
        const curSelectData = nodes.filter(p => !p.disabled && (p.checked || p.noChildChecked));
    ...
    privateGeneratePosition() {
      let headerList = document.querySelectorAll(".columns-panel__content_box_header");
      let wrap = document.querySelector(".columns-panel__content_right");
      ...
      this.flattenData.forEach(item => {
        if (item.data[this.props.controlList]) {
          item.data[this.props.controlList].forEach(id => {
            let disabledNode = this.flattenData.find(val => val.data[this.props.key] === id);
            if (disabledNode) {
